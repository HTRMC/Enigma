...
static_assert(sizeof(SSAOPushBlock) == 32);

RTGIPass::RTGIPass(gfx::Device& device, gfx::Allocator& allocator)
    : m_device(&device)
...
void RTGIPass::buildPipeline(gfx::ShaderManager& shaderManager,
    ...
    if (m_useRT) {
        m_rgenPath  = Paths::shaderSourceDir() / "rt" / "gi.rgen.hlsl";
        m_rchitPath = Paths::shaderSourceDir() / "rt" / "gi.rchit.hlsl";
        ...
        ci.pushConstantSize  = sizeof(RTGIPushBlock);
        ...
        ENIGMA_LOG_INFO("[rt-gi] RT pipeline built");
    } else {
        m_ssaoPath = Paths::shaderSourceDir() / "ssao.hlsl";

        VkShaderModule vert = shaderManager.compile(m_ssaoPath, gfx::ShaderManager::Stage::Vertex,   "VSMain");
        VkShaderModule frag = shaderManager.compile(m_ssaoPath, gfx::ShaderManager::Stage::Fragment, "PSMain");
        ...
        ci.pushConstantSize      = sizeof(SSAOPushBlock);
        ...
        ENIGMA_LOG_INFO("[rt-gi] SSAO fallback pipeline built");
    }
}

void RTGIPass::allocate(VkExtent2D extent) {
...
void RTGIPass::record(VkCommandBuffer cmd,
    ...
    if (m_useRT) {
        ...
        // SSAO fallback: no-op placeholder; integration into render graph
        // deferred to when the render graph supports raster-to-storage passes.
        (void)globalSet;
        (void)normalSlot;
        (void)depthSlot;
        (void)cameraSlot;
...
void RTGIPass::registerHotReload(gfx::ShaderHotReload& reloader) {
    if (m_useRT) {
        reloader.watchGroup({m_rgenPath, m_rchitPath, m_rmissPath}, [this]() {
            ENIGMA_LOG_INFO("[rt-gi] hot-reload triggered (RT shaders)");
        });
    } else {
        reloader.watchGroup({m_ssaoPath}, [this]() {
            ENIGMA_LOG_INFO("[rt-gi] hot-reload triggered (SSAO shader)");
        });
    }
}

void RTGIPass::destroyOutput() {
