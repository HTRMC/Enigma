...
static_assert(sizeof(WetRoadFallbackPushBlock) == 32);

WetRoadPass::WetRoadPass(gfx::Device& device, gfx::Allocator& allocator)
    : m_device(&device)
    , m_allocator(&allocator)
    , m_useRT(device.gpuTier() >= gfx::GpuTier::Recommended) {}

WetRoadPass::~WetRoadPass() {
    destroyOutput();
    delete m_rtPipeline;
    delete m_fallbackPipeline;
}

void WetRoadPass::buildPipeline(gfx::ShaderManager& shaderManager,
                                 VkDescriptorSetLayout globalSetLayout) {
    m_shaderManager  = &shaderManager;
    m_globalSetLayout = globalSetLayout;

    if (m_useRT) {
        m_rgenPath  = Paths::shaderSourceDir() / "rt" / "wetroad.rgen.hlsl";
        ...
        ci.pushConstantSize  = sizeof(WetRoadPushBlock);
        ...
        ENIGMA_LOG_INFO("[wet-road] RT pipeline built");
    } else {
        m_fallbackPath = Paths::shaderSourceDir() / "wetroad_fallback.hlsl";

        VkShaderModule vert = shaderManager.compile(m_fallbackPath, gfx::ShaderManager::Stage::Vertex,   "VSMain");
        VkShaderModule frag = shaderManager.compile(m_fallbackPath, gfx::ShaderManager::Stage::Fragment, "PSMain");
        ...
        ci.pushConstantSize      = sizeof(WetRoadFallbackPushBlock);
        ...
        ENIGMA_LOG_INFO("[wet-road] screen-space fallback pipeline built");
    }
}

void WetRoadPass::allocate(VkExtent2D extent) {
...
void WetRoadPass::record(VkCommandBuffer cmd,
    ...
    if (m_useRT) {
        ...
        // Screen-space planar reflection fallback: no-op placeholder.
        (void)globalSet;
        (void)normalSlot;
        (void)depthSlot;
        (void)cameraSlot;
...
void WetRoadPass::registerHotReload(gfx::ShaderHotReload& reloader) {
    if (m_useRT) {
        reloader.watchGroup({m_rgenPath, m_rchitPath, m_rmissPath}, [this]() {
            ENIGMA_LOG_INFO("[wet-road] hot-reload triggered (RT shaders)");
        });
    } else {
        reloader.watchGroup({m_fallbackPath}, [this]() {
            ENIGMA_LOG_INFO("[wet-road] hot-reload triggered (fallback shader)");
        });
    }
}

void WetRoadPass::destroyOutput() {
